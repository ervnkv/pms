...
export const FieldStatus = observer(({ model }: FieldStatusProps) => {
  return (
    <FormControl variant='filled'>
      ...
      <Select
        value={model.form.status.value ?? ''}
        labelId="status"
      >
        {model.form.status.options.map(status => (
          <MenuItem key={status} value={status} onClick={() => model.change('status', status)}>{status}</MenuItem>
        ))}
      </Select>
      <FormHelperText error={model.form.status.isError}>{model.form.status.errorText || ' '}</FormHelperText>
    </FormControl>
  );
});
